...
# Fix the incorrect replacement inside pendingText
code = code.replace("fontSize: `${Math.max(12, Math.round((ann.fontSize || 24) * zoomLevel))}px`,", "fontSize: `${Math.max(12, Math.round(24 * zoomLevel))}px`,")

# We want to replace the ENTIRE pendingText render block with the draggable wrapper!
target_block = """                  {pendingText && (
                    <input
                      autoFocus
                      type="text"
                      dir="auto"
...
if target_block in code:
    ...
    with open('src/components/study/PdfNotebookViewer.tsx', 'w', encoding='utf-8') as f:
        f.write(code)
    print('Replaced pendingText block successfully')
else:
    print('Could not find pendingText target block')

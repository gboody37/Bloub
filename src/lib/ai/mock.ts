/**
 * Mock Gemini LLM Test Double & Offline Engine
 * 
 * Provides deterministic, offline-capable AI quiz generation, hint synthesis,
 * answer evaluation, and connection testing for Vibe Todos study workflows.
 */
...
export class MockGeminiClient {
  ...
  static async generateQuiz(request: QuizGenerateRequest): Promise<QuizResponse> {
    ...
    if (apiKey === 'REVOKED_OR_INVALID_KEY' || apiKey === 'INVALID_KEY') {
      const err: CustomHttpError = new Error('INVALID_API_KEY: Provided API key is invalid or unauthorized.');
      err.statusCode = 401;
      throw err;
    }

    if (apiKey === 'TRIGGER_RATE_LIMIT') {
      const err: CustomHttpError = new Error('RATE_LIMIT_EXCEEDED: Google AI Gemini rate limit reached. Retry in 5000ms.');
      err.statusCode = 429;
    ...
    if (apiKey === 'TRIGGER_MALFORMED_OUTPUT') {
      const err: CustomHttpError = new Error('MALFORMED_OUTPUT: Unable to parse structured JSON from LLM response.');
      err.statusCode = 502;
      throw err;
    }
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (selectedType === 'multiple_choice') {
        ...
        const isArch = lowerNote.includes('agent') || lowerNote.includes('architecture');
        const isMCP = lowerNote.includes('mcp') || lowerNote.includes('protocol');
        const isBTEC = lowerNote.includes('btec') || lowerNote.includes('jordan');
  ...
  static async evaluateAnswer(request: EvaluateRequest): Promise<EvaluationResponse> {
    const { apiKey, userAnswer } = request;
    if (!apiKey) {
   ...
   */
  static async testConnection(apiKey: string): Promise<ConnectionTestResult> {

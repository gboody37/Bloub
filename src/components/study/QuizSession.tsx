...
export default function QuizSession({ note, apiKey, isDark = true, onClose, triggerMascot }: QuizSessionProps) {
  ...
  if (loading) {
    return (
      <div className={`flex flex-col items-center justify-center h-full w-full p-12 ${isDark ? 'text-slate-200' : 'text-gray-800'}`}>
        <div className="w-16 h-16 mb-6 relative">
          <Brain className="w-full h-full text-purple-500 animate-pulse" />
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-full w-full p-12 text-center">
        ...
        <button onClick={generateQuiz} className="px-6 py-2 bg-purple-600 text-white rounded-xl font-bold">Try Again</button>
  ...
  if (isFinished) {
    return (
      <div className={`flex flex-col items-center justify-center h-full w-full p-8 animate-in fade-in zoom-in-95 ${isDark ? 'text-white' : 'text-slate-900'}`}>
        ...
        <div className="flex gap-4">
          <button onClick={onClose} className="px-6 py-3 rounded-2xl bg-slate-800 text-white font-bold hover:bg-slate-700 transition-all">
            Back to Note
          </button>
          <button onClick={generateQuiz} className="px-6 py-3 rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold shadow-lg shadow-purple-600/30 hover:scale-105 active:scale-95 transition-all">
            Generate New Quiz
          </button>
        </div>
      </div>
  ...
  return (
    <div className={`flex flex-col h-full w-full relative ${isDark ? 'text-slate-200' : 'text-slate-900'}`}>
      {/* Header */}
        ...
        <div className="flex items-center gap-3">
          <div className="flex gap-1">
            {quizData?.questions.map((_, i) => (
              <div key={i} className={`w-8 h-1.5 rounded-full ${i === currentIdx ? 'bg-purple-500' : i < currentIdx ? (answers[quizData.questions[i].id]?.isCorrect ? 'bg-green-500' : 'bg-red-500') : isDark ? 'bg-slate-800' : 'bg-gray-200'}`} />
            ))}
          </div>
        ...
        {currentQ && (
          <div className="animate-in slide-in-from-right-8 fade-in duration-300">
            ...
            <div className="flex items-center justify-between mb-4">
              <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider ${isDark ? 'bg-purple-500/10 text-purple-400' : 'bg-purple-100 text-purple-700'}`}>
            ...
            <div className="space-y-4">
              {currentQ.type === 'multiple_choice' || currentQ.type === 'true_false' ? (
                currentQ.options?.map((opt, idx) => {
                  ...
                  let stateClass = isDark ? 'bg-slate-800/50 hover:bg-slate-800 border-transparent' : 'bg-gray-50 hover:bg-gray-100 border-transparent';
                ...
                <form onSubmit={handleShortAnswerSubmit} className="space-y-4">
                  <textarea
                    ...
                    className={`w-full p-5 rounded-2xl border-2 min-h-[120px] resize-none outline-none transition-all ${isDark ? 'bg-slate-800/50 border-slate-700 focus:border-purple-500' : 'bg-gray-50 border-gray-200 focus:border-purple-500'}`}
                  />
                  {!currentAnswer && (
                    <button 
                      type="submit" 
                      disabled={isEvaluating}
                      className="px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-xl flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50"
                    >
            ...
            {!currentAnswer && !hint && (
              <div className="mt-8">
                <button onClick={getHint} disabled={isGettingHint} className="text-xs font-semibold text-purple-500 flex items-center gap-1.5 opacity-75 hover:opacity-100">
